...
pub fn save_tasks_to_file(backlog_list: Vec<Task>, inprogress_list: Vec<Task>, done_list: Vec<Task>) {
    ...
    file.write_all(backlog.expect("Should be able to read").as_bytes()).unwrap();
    file.write_all(inprogress.expect("Should be able to read").as_bytes()).unwrap();
    file.write_all(done.expect("Should be able to read").as_bytes()).unwrap();
